...
if (!seed) {
  ...
  document.title = 'Sundrop - ' + seed;
}
var newPageTitle = 'Sundrop - ' + seed;

// 'Autoplay' is also a url parameter - The value is the ms for getting a new sundrop.
// https://anemy.github.io/sundrop/?autoplay=8000
...
function moveDot(x, y, size, paintColor) {
  ...
  var direct = noise((x + noiseOffsetX) * noiseScale, (y + noiseOffsetY) * noiseScale, 1 * noiseScale); // Math.random();
  var amtMult = noise((x + noiseOffsetX2) * noiseScale, (y + noiseOffsetY2) * noiseScale, 1 * noiseScale);
  var amt = dotMove * amtMult;
  
  if (amtMult > moveThresh) {
    dx += Math.cos(direct) * amt * xMult;
    dy += Math.sin(direct) * amt * yMult;
    
    if (showConnect && Math.random() < connectShowChance) {
      stroke(paintColor);
      line(x, y, dx, dy);
      noStroke();
    }
...
function paintInner({
  ...
  for (var m = 0; m < amount; m++) {
    var appliedVariation = (variation * Math.random()) - (variation * Math.random());
    var dist = radius - (radius * Math.random() * Math.random());
    if (outboost > 1) {
      dist = dist + (Math.random() > 0.7 * dist)
    }
    var dx = x + (Math.cos(moonLight + appliedVariation) * dist);

    var dy = y + (Math.sin(moonLight + appliedVariation) * dist);
    
    moveDot(dx, dy, paintSize, paintColor);
  }
}
...
function sundrop() {
  ...
  var rows = 1 + Math.floor(50 * Math.random() * Math.random() * Math.random() * Math.random() * Math.random() * Math.random() * Math.random()); // 2 + Math.floor(Math.random() * 60);
  var columns = rows;
  ...
  var palette = Math.random() > 0.7 ? [
    ...
    color(180, 222, 193),
    color(23, 23, 29)
  ] : manyColors[Math.floor(Math.random() * manyColors.length)];
  ...
  noStroke();
  strokeWeight(Math.random() > 0.5 ? paintSize : ((canvasWidth / 1600) + (Math.random() * Math.random() * Math.random())));
  
  const moonLight = Math.random() * Math.PI * 2;

      
  var boxColor = palette[Math.floor(Math.random() * palette.length)];

  var inner = palette[Math.floor(Math.random() * palette.length)];
  
  var innerVar = (Math.random() * 20);

  var highlight = palette[Math.floor(Math.random() * palette.length)];

  for (var i = 0; i < rows; i ++) {
    for (var k = 0; k < columns; k++) {
      fill(boxColor);

      var x = ((i / columns) * canvasWidth) + (padding * width);
      var y = ((k / rows) * canvasHeight) + (padding * height);
      ...
      paintInner({
        ...
        radius,
        amount: 1 * baseAmt * dotMult,
        moonLight: moonLight + Math.PI,
        variation: Math.random() > 0.1 ? innerVar : Math.PI / (Math.random() * 20)
      });
      
      fill(highlight);

      paintInner({
        paintColor: highlight,
        x: x + radius,
        y: y + radius,
        radius: radius + ((padding * 1.25 * width) * Math.random()),
        amount: 10000 * dotMult,
        moonLight: moonLight,
        variation: Math.PI * (8 * Math.random() * Math.random()),
        outboost: 5
      });
    }
  }
